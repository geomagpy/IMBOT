        ...
        if dataset.get('modification') in ['updated but already accepted']:
            ...
            if dataset.get('resolution') in ['minute']:
                ...
                maildict = {'subject': "Submission one-{} {}, {} moved to step2".format(dataset.get('resolution'),dataset.get('obscode'),dataset.get('year')),
                            'text': "Dear data provider,\nplease accept our apologies for duplicate and delayed postings. INTERMAGNET DD is updating the notification system.\n\nYour data submission has successfully passed the first review step and has been moved to step2 for cross-checking.\nSincerely,\n     IMBOT",
                            'to': receivers, 'from': [adminmail] }
                if debug or nomail:
                    print(maildict)
                else:
            ...
            maildict = {'subject': "Submission one-{} {}, {} moved to step3".format(dataset.get('resolution'),dataset.get('obscode'),dataset.get('year')),
                        'text': "Dear data provider,\nplease accept our apologies for duplicate and delayed postings. INTERMAGNET DD is updating the notification system.\n\nThank you for submitting the {a} {b} annual dataset. After careful cross-check that includes reviewing the quality of data, baseline adoptions, file formats and metadata, we are pleased to report that {a} {b} dataset meets INTERMAGNET standards and has been accepted as definitive data.\n{a} {b} dataset will be published on the INTERMAGNET website within 24h, it will be included in INTERMAGNET Reference Data Set (IRDS{b}}).\nIn the name of INTERMAGNET, we want to congratulate you and your staff for the high standards you have achieved in geomagnetic observatory operations and data production. I would also like to thank you for your continued participation in the INTERMAGNET program. Your contribution helps INTERMAGNET to fulfil its mission to provide science with a valuable worldwide resource for studying the properties of the Earth, its environment and hazards.\nIn case you need an updated official certificate by INTERMAGNET please contact the chairs of INTERMAGNET DD subcommittee.\n\nYours sincerely,\n    INTERMAGNET Definitive data subcommittee".format(a=dataset.get('obscode'),b=dataset.get('year')),
                        'to': receivers, 'from': [adminmail] }
            if debug or nomail:
                print(maildict)
            else:
